...
WITH ordered AS (
	SELECT
		*,
		COUNT(date) OVER (PARTITION BY "9_shape") AS instances,
		ROW_NUMBER() OVER (PARTITION BY "9_shape" ORDER BY date ASC) AS instance_count
...
JOIN (
	SELECT
		*
	FROM ordered
	WHERE instances = instance_count
) AS latest
ON o."9_shape" = latest."9_shape"
